...
| `RooRealVar`, `RooLinkedList`, `RooAbsBinning`, `RooRefArray`, `RooCategory` | [RooFit](RooFit.md) — the five RooFit classes with a hand-written `Streamer` that any workspace reaches, and the shared-properties tails of `RooRealVar` and of `RooCategory` below version 3 |
