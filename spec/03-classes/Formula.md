...
One class name on disk covers two unrelated C++ classes. A file written before
ROOT 6.04 and a file written after it both contain a record whose class is
`TFormula`, and the member lists have nothing in common. Only the version word
separates them, and the discontinuity is deliberate.
...
cannot get from the file is *which class it is looking at*, and that is what this
document records.
...
| `TF1Data` | 7 | 6.02 – current | likewise, as `ROOT::v5::TF1Data`. The `ClassDef` spells it `TF1Data`, unqualified |
...
1)`, the same version number the old class had used since ROOT 2. It was raised
to 9 in `803a72ec9b9` (2014-12-03), then to 10 in `6a1710b9f39` (2015-05-07).

Both the introduction and the change to 9 fall inside the same first release,
...
**`TFormula` at version ≤ 8 is the old class, and ≥ 9 is the new one.** A reader
may use that as a rule. No other rule is available, because nothing else in the
record distinguishes them: the name and the `TNamed` base are the same, and the
checksum is useful only if you already have both schemas.
...
The old class stores a pre-parsed operator stream; the new one stores the source
text and compiles it. `fNdim` and `fNumber` are the only members that keep both
their name and their meaning. `fParams` keeps the name but means something else:
a counted array of values in v8 and a name-to-index map in v14.
...
The limits row is the trap: the two members are adjacent `Double_t`s whose order
is swapped between the versions, so reading a v7 record with a v12 layout puts a
plausible value in each.
...
That second path branches again
...
The `onfile_class` detour cannot be seen from outside. It exists because ROOT
wants the old bytes in a different C++ object; the bytes themselves are what the
file's `TF1` v5–v7 streamer info says. A third-party reader that decodes records
with the info in the file reads them correctly without knowing that
`ROOT::v5::TF1Data` exists.

> For the same reason the two `v5` classes are not in
> [Hand-written streamers](../99-appendix/HandWrittenStreamers.md)'s `custom`
> list, though a first pass put them there: their `ReadClassBuffer` call is in a
> sibling overload, and the overloads form one streamer.

## 4. The layouts that are hand-decoded

`TFormula` v ≤ 3 and `TF1` v ≤ 4 are read member by member with no reference to a
streamer info, and they predate schema evolution, so a file that old may have no
...
They are summarised here rather than transcribed, so the gap is not mistaken for
coverage:
...
- `TFormula` v ≤ 3: `TNamed`, `fNdim`, `fNumber`, then `fNval` (v > 1) and
  ...
  This layout cannot be reached from a `TFormula` key: ROOT 6's
  `TFormula::Streamer` has no branch for v ≤ 3 (it reports an error), and the v5
  path that hand-decodes it is entered only from `TF1Data::Streamer` at `TF1`
  v ≤ 4.
...
The histogram at v1 is on disk and is read and discarded; a reader must still
consume it.
...
Checked by `tools/check_invariants.py` wherever a file has the info. The v ≤ 7
half of invariant 2 and the v ≤ 8 half of invariant 3 are exercised by
`uproot-issue-181.root` in `gen/foreign/`. Under the standing provenance rule that
file is a lead rather than evidence, but its ROOT version word is 53436, so
ROOT 5.34/36 wrote it, and the claim is in any case about the file's agreement
with itself.
...
| 2 | `TFormula`'s class version identifies its schema | It identifies the schema *and the class*. Versions 1–8 and 9–14 are different C++ classes with different member lists under one name, the only such case the project has found |
