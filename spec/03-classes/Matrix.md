...
`TMatrixTSym` is the one class in this family whose bytes a reader has to know,
and it is worth a document because of *how* it diverges rather than how much.
Its `Streamer` calls `ReadClassBuffer` like an ordinary class — and then reads
the matrix elements itself, outside the byte count, from a layout no streamer
info in any file describes.

[Hand-written streamers §3](../99-appendix/HandWrittenStreamers.md) calls that
kind `extending`. Three classes in ROOT are that kind; this is the one a physics
file is likely to contain, because a covariance matrix is symmetric.

## 1. Class versions

The concrete names are typedefs — `TMatrixDSym` is `TMatrixTSym<Double_t>`
(`root/math/matrix/inc/TMatrixDSymfwd.h:23`), `TMatrixD` is
`TMatrixT<Double_t>`, `TVectorD` is `TVectorT<Double_t>` — and it is the
**template spelling** that appears in a file, as a class name and as a streamer
info name.

Each is a template, and a file names the specialization — `TMatrixTBase<double>`
— where the `ClassDef` macro names the template, which is what this table checks:
...
The framed part — everything from the version word to `fTol` — is exactly what
the streamer info for `TMatrixTBase<T>` in the same file describes, read as
...
(`root/math/matrix/src/TMatrixTSym.cxx:2054`). So:
...
- **the file carries no streamer info for `TMatrixTSym<T>` at all.** Recording an
...
nothing, the same position [`TBasket`](../04-ttree/TBasket.md) puts it in, and
must dispatch on the name.
...
- the lower-left triangle is **reconstructed** by ROOT after reading
  ...
  full square and is not the number of values on disk.

The element width is the template argument's, and nothing in the record states
it: `symf` in the same fixture is a `TMatrixTSym<float>` whose three elements are
4 bytes each — and whose `fTol` is 4 bytes too, since `fTol` is declared
`Element`. A reader gets the width from the class name.

### 2.4 Outside its own byte count, inside every other

The byte count at 329 is 44 and covers the framed part only. That has two
...
- **`CheckByteCount` cannot detect the mistake.** A reader that stops where the
  ...
  error. Five `TMatrixTSym<double>` records in `uproot-issue-359.root`
  (ROOT 5.34/34) do exactly that: 48 of 3528, 48 of 13736 — and `PLAN.md` §9.8
  had filed it as a missing class rather than as the symptom of a wrong claim.
- **Every enclosing frame does count them.** In `classes/matrix` the same class
  appears inside a `TObjArray`: the pointer-streamed slot at 882 has a byte count
  of 96 which includes the triangle, while the `TMatrixTBase` byte count at 910
  is 44 and does not. So a file containing one is never *inconsistent* — the
  bytes are accounted for by the object that holds it.

That second point has a sharp edge. `TBufferFile::SkipObjectAny` skips an object
by seeking to `start + count + 4` (`root/io/io/src/TBufferFile.cxx:2499-2503`),
so **an object of this kind cannot be skipped by its byte count**: the seek lands
...
class's own frame, and it is not. No file here demonstrates it — it needs a class
that *dropped* such a member — and it is recorded as a bug candidate in
`PLAN.md` §7.1 rather than as an established fact.
...
read nothing further, so they are ordinary streamer-info-driven objects with an
info in the file and their own class version on disk.

`classes/matrix` pins both, as the contrast that makes §2 legible:
...
1. Read the byte count and version word as
   ...
   `TMatrixTBase<T>`'s class version, and it is what selects the info.
...
Against ROOT's own documentation and against this specification as it stood.
...
`uproot-issue-359.root` in `gen/foreign/` is the independent witness: five
`TMatrixTSym<double>` records written by ROOT 5.34/34, at 29 × 29 and 58 × 58,
each accounting to the byte exactly as §2 gives it. `linearIO.root` in
`gen/cern/` carries `TMatrixT<float>`, `TMatrixT<double>`, `TMatrixTSparse` and
`TVectorT` infos, written by ROOT 5.05/01.
