...
| **ROOT's own value is wrong** | four `pair<…>` instances | A `pair`'s checksum can be computed before its members are known and is then cached forever (`root/core/meta/src/TClass.cxx:6655-6666`); `data/serialization/pairs.root` has three distinct pairs all carrying `0x0b5fb752`, and the fourth pair in the same file is correct and recomputable. `data/classes/roofit.root`, written by an unrelated program, carries the same value on a fifth layout, `pair<string,vector<int> >` — so `0x0b5fb752` is a constant, not one file's accident |
