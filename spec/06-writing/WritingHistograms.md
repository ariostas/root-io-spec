...
The whole of this document is checked by byte comparison against ROOT. Seven
records, in two pairs of files:
...
`data/written/histogram.root` and `data/written/th2-profile.root` — and every
comparison above is asserted by `tools/test_write.py`. The files differ only in
...
of them inserts its own members **after** the `TH1` base rather than around it,
which is the whole reason they need procedures of their own.
...
Three frames, not two. A `TH2F` record is a byte count and a version word around
a `TH2`, which is another around a `TH1`, and `TH2`'s four own members sit
**between** the `TH1` frame closing and the `TArray` base opening:
...
(`root/hist/hist/src/TH2.cxx:2836`, `:4004`, `:4277`) — so the bytes are exactly
what the three streamer infos describe, and unlike `TH1` none of them does any
fixup on read. `TH2D` differs from `TH2F` in the `TArray` base and nothing else,
and carries **the same class version, 4**.
...
row*. The four corners are double-flow cells that no single-axis reasoning
reaches.

That layout is why the statistics cover a **rectangle** rather than "every cell
but the first and last". `TH2::Fill` increments `fEntries` before it knows where
the fill landed (`root/hist/hist/src/TH2.cxx:391`), and then skips all seven sums
if `binx` is 0 or above `nx` *or* `biny` is 0 or above `ny`
(`root/hist/hist/src/TH2.cxx:398-403`). So `fEntries` counts a fill that is out
of range in either axis and `fTsumw` does not, exactly as in one dimension — and
a writer computing the sums from cells has to iterate `1 <= binx <= nx` and
...
| Member | Type | Value | Kind |
|---|---|---|---|
| `fScalefactor` | `f64` | **1.0** | free — and dead, below |
| `fTsumwy` | `f64` | Σ w·y over in-range fills | derived |
| `fTsumwy2` | `f64` | Σ w·y² | derived |
| `fTsumwxy` | `f64` | Σ w·x·y | derived |

Declaration order, `root/hist/hist/inc/TH2.h:33-36`, and all four are
persistent; `TH2` adds no transient members and no enum, so its checksum folds
four plain doubles onto the `TH1` base's.
...
setter and no arithmetic anywhere in ROOT — JSROOT hardcodes the same 1 when it
synthesises a `TH2` (`root/js/modules/core.mjs:1444`). Write 1.0: nothing breaks
if you do not, and nothing else compares byte for byte if you do.

**`fTsumwxy` has exactly one reader.** `TH2::GetCovariance` is the only place
`stats[6]` is used (`root/hist/hist/src/TH2.cxx:1156`), and
`GetCorrelationFactor` through it. `fTsumwy` and `fTsumwy2` reach
`GetMean(2)` and `GetStdDev(2)`, which index the same statistics array with
`ax[3] = {2,4,7}` (`root/hist/hist/src/TH1.cxx:7674`, `:7746`), and a Y
projection copies them down into the projection's x sums
(`root/hist/hist/src/TH2.cxx:2344-2346`).
...
centres when `(fTsumw == 0 && fEntries > 0)` holds, or when either axis carries
`kAxisRange` (`root/hist/hist/src/TH2.cxx:1230`). Only on the other branch does it
hand back what the record says. A genuinely empty histogram is safe, because
`fEntries` is 0 there too.

For a writer that means the sums are not optional in the way `fSumw2` is. Write
`fEntries` 6 and leave `fTsumw` at 0 and ROOT will silently discard `fTsumwy`,
`fTsumwxy` and the rest and report a mean computed from the bins — which is the
bin-centre approximation of §5 arrived at by accident, with no diagnostic. The
same condition guards `TProfile::GetStats`, with one difference worth knowing
(§8.5).

## 8. `TProfile`

A `TProfile` is a `TH1D` with **four parallel arrays per cell**, of which the
`TH1D`'s own `TArrayD` base is one. Nothing it stores is a bin content:
...
The number ROOT reports for a bin is `fArray[i] / fBinEntries[i]`, computed on
...
layout nor the checksum — the same trap as `TH1`'s four transient members (§3),
in a place a writer is more likely to miss because it is a single `Bool_t` among
doubles.

`fErrorMode` is the one **enum** in either chain besides `TH1`'s two, and its
type is a file-scope `EErrorType` rather than a nested one
...
Treat it as a plain `int` and the checksum comes out as something other than
`0x4bedee54`, which ROOT reports through `BuildCheck`.
...
(`root/hist/hist/src/TProfileHelper.h:138-140`). So:

- **`fSumw2` always has exactly `fNcells` entries** in a `TProfile`, where in a
  plain `TH1` it is empty until something asks for it (§5.1). This is the one
  ...
  pointer. Measured on 6.40.04, with a file this project wrote deliberately:
...
- **`fBinSumw2` is empty until a weight other than 1 arrives**, at which point
  `Fill` calls `Sumw2()` for you (`root/hist/hist/src/TProfile.cxx:751`) — and
  `Sumw2()` pre-fills it from `fBinEntries`
  (`root/hist/hist/src/TProfileHelper.h:553-557`), so a profile that was filled
  unweighted and then weighted carries a consistent array rather than a hole.

A wrong **length** for `fBinSumw2` is worse than a wrong value:
`GetBinEffectiveEntries` tests `fBinSumw2.fN != fNcells` and, if it differs,
...
Two consequences for a writer. First, **`fBinEntries` is not a count**, whatever
its comment says (§12): it is a sum of weights, and `GetBinContent` divides by
it, so treating it as an integer count of fills makes every weighted bin wrong.
Second, `GetBinError` returns 0 whenever `sum` is 0 (`:717`), which is also what
an empty bin returns — so a cell that holds data with `fBinEntries` 0 reads as
...
range of zero width cannot be stated. Two things follow. A writer that means "no
range" must write two **equal** values — 0 and 0 is ROOT's pair — and a writer
carrying a range over from somewhere else has to check that its two limits differ,
or the filter it believes it recorded is simply off. And the NaN rejection rides on
the same guard, so a profile with no range accepts a NaN `y` into `fArray`,
`fSumw2`, `fTsumwy` and `fTsumwy2`.

When the filter does reject a fill it returns **before `fEntries++`**
(`root/hist/hist/src/TProfile.cxx:682-686`), which is the opposite of `TH2`,
where an out-of-range fill still counts as an entry (§7.1). `data/classes/th2-profile.root`'s
`p2` has one of each: `fEntries` is 3 after four `Fill` calls.

### 8.5 Only `fEntries` is unrecoverable

A profile is the one histogram whose statistics a writer can almost entirely
*derive*, because the per-cell arrays already hold what a `TH1` throws away:
...
`rootwrite.stats_from_profile` does exactly that, and for both profiles in the
reference file it reproduces ROOT's six sums; only `fEntries` is supplied, and
only for `p2`, where three fills of weight 3, 0.5 and 0.5 sum to 4.

Two repair paths make a lazy writer's output *look* right while being something
else. `TProfile::GetStats` recomputes all six sums from the arrays when
`fTsumw == 0` — and its version of the condition has `&& fEntries > 0`
**commented out in the source** (`root/hist/hist/src/TProfile.cxx:958`), so
unlike `TH2` (§7.3) an empty `fTsumw` diverts even when `fEntries` is 0. And on
the fast path, if `fTsumwy` and `fTsumwy2` are *both* 0, ROOT fills them in from
`fArray` and `fSumw2` over the displayed range, casting away `const` to do it
(`:980-987`, whose comment blames `TProfile` versions ≤ 3). A writer that leaves
those two at 0 therefore gets range-dependent values back, not zeros.
...
not an info**: a `TList` named `listOfRules` carrying one `TObjString`, the I/O
...
readable by anything but ROOT has to emit every one:
...
Eighteen for a file of `TH2F`, `TH2D` and `TProfile`, which is the same set with
three more classes and the concrete ones changed:
...
beside the other bases: the order is the order the classes were registered, so
...
[Element lists](ElementLists.md) publishes every one of the twenty-two distinct
classes, member by member, with the checksum beside it. Five things in the lists
are not obvious:
...
- **No `TArray` info is written at all**, for `TArrayF`, `TArrayD` or `TArray`
  ...
  [Element lists §8](ElementLists.md#8-three-classes-no-file-describes). A `TH2F`
...
- **A `TProfile` brings a nineteenth entry that is not an info**, the
  `listOfRules` of §8.6. Omitting it is what a writer does; tolerating it is what
  a reader must do.
...
([StreamerInfo §11.1](../02-serialization/StreamerInfo.md#111-an-enum-member-is-recognisable-and-it-changes-the-value)).
`TProfile`'s `0x4bedee54` needs the same rule for `fErrorMode`, and `TH2`'s
`0x0182347f` needs none — it folds four plain doubles onto `TH1`'s.
...
8. In a `TProfile`, `fBinEntries` has exactly `fNcells` entries and `fSumw2` is
   **never** empty — it is the one histogram class where an empty `fSumw2` is not
   a legal "use Poisson errors", and the one invariant here that ROOT enforces by
