...
| Writing — the container, an object, `TH1`/`TH2`/`TProfile`, a flat `TTree` | [written](06-writing/index.md) |
...
Behind it: **75 reference files with 1950 byte-level assertions, 1499 source
...
The writing layer adds eight files this project *did* write, with 398 assertions of
their own. Seven of them are the strongest check here: **every object-bearing
record in them is byte-identical to the one ROOT wrote** — a `TH1F`, a `TH1D`, a
`TH2F`, a `TH2D`, two `TProfile`s, three `TTree`s, **nine** `TBasket`s, a
