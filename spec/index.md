...
Behind it: **72 reference files with 1838 byte-level assertions, 1402 source
...
`TTree`, two `TBasket`s, a `StreamerInfo` record of fifteen class descriptions,
and the eighteen of a tree file bar the one entry a file written today cannot
use.
...
- **On the write side**, everything [Writing §4](06-writing/index.md#4-what-is-not-specified)
  ...
  element list of any class beyond the twenty-seven
  [published](06-writing/ElementLists.md), and ROOT's policy choices. The first
  three are out of scope by decision; the rest are limits of what has been written
  so far, and `PLAN.md` §8.5 ranks them.
