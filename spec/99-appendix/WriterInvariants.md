...
It is an index, not the normative text. Each row points at the document that
defines the rule.

## 1. How to use it

There are three sources of enforcement, and they differ greatly:

| Column | Meaning |
|---|---|
| **checked** | `tools/check_invariants.py` verifies it over every file it is given, including the ones this project writes |
| **ROOT** | ROOT reports something when it is violated; the message is in the linked document |
...
The invariant checker works on any file, including ones this project had nothing
to do with; it was written for that purpose.
...
| A key-list record's key holds the `fSeekDir` of the directory that owns the list, not its parent's | Directory 9.12, WritingFiles 5.4 | checked |
...
| A directory record's payload is the length implied by its class version, its offset width and the file's `fVersion`; the version word is a sum of two independent things | Directory 9.15, 3.1 | checked |
| `fBEGIN` is at least the size of the header its own `fVersion` selects: 63 bytes small, 75 large | FileHeader 10.11, WritingFiles 14.17 | checked, but nothing on ROOT's side, which writes the header over the first record instead |
...
| The transitive closure: every base class and every contained class has an info too, unless its `Streamer` is hand-written or forwarding | WritingObjects 8.2 | ROOT, loudly, but only when reading without the base compiled in: `BuildOld` skips the base, `CheckByteCount` reports the short read, and the class comes out the wrong size |
...
| On every axis, `fXbins` is empty or has `fNbins + 1` entries, whose first and last are `fXmin` and `fXmax` | checked |
...
| `fMaximum` and `fMinimum` are `-1111` unless a range was set | nothing; the histogram just draws wrongly |
| The statistics are consistent with each other: `fTsumw <= fEntries` for unit weights | nothing |
| `fTsumw` is non-zero whenever `fEntries` is, or ROOT recomputes every sum from the bins | nothing, and the values it then reports are the bin-centre approximation |
| In a `TProfile`, `fBinEntries` holds exactly `fNcells` values and `fSumw2` is never empty | checked; **ROOT segfaults** in `GetBinError` on an empty `fSumw2`, after reading the entries and the contents correctly |
| In a `TProfile`, `fBinSumw2` is empty or holds exactly `fNcells` values; any other length is dropped on the first call that reads it | checked |
...
| Invariant | Who notices |
|---|---|
| `fNpoints >= 0`, and each counted array's flag byte is either 1 with `fNpoints` doubles after it, or 0 with `fNpoints` also 0 | checked in two halves: the flag byte here, the length by the byte count |
...
| `fFunctions` points at an empty `TList` rather than being null | nothing; a null reads back without a warning, and ROOT never writes one |
...
| Invariant | Who notices |
|---|---|
| `fBasketSeek[i]` names a record whose key reports the same `fSeekKey` | ROOT: the only consistency check it makes on a tree |
...
| `fClusterRangeEnd` and `fClusterSize` hold exactly `fNClusterRange` values, and their is-present flag agrees with the count | checked; a count that disagrees desynchronises the record, so nothing after `fBranches` parses |
...
| On a `TLeafC`, `fLen` and `fMaximum` are equal and are the length of the longest string in the file plus one | checked as `fLen <= fMaximum`, and `fMaximum` is checked against every string in the baskets; the equality is the writer's half |
...
| A branch holding a `TLeafC` has `fEntryOffsetLen` non-zero and its baskets have the offset array | checked |

## 7. The ones ROOT does not notice

Violations that produce a file ROOT reads without a warning, and that another
reader may reject or misread. Each is specified where the tables above say.
Several of them are caught by `tools/check_invariants.py` (the column defined in
§1 says which), so "nothing" here means nothing in ROOT, not nothing at all.
...
1. **`fObjlen` inconsistent with the stored length.** There is no codec flag; the
   ...
   stored length passes a zip stream to the streamer.
2. **A key image in the key list that disagrees with the record it points at.**
   ROOT frames the read from the image.
...
4. **A class back-reference with the wrong position.** It has no effect until the
   second occurrence of a class, so small records are fine and large ones are not.
...
9. **A `TLeafC`'s `fLen` smaller than its longest string.** ROOT sizes the read
   buffer from it and truncates the value silently, while the bytes in the basket
   are complete. A fast merge (`hadd`'s default) produces this.
...
10. **A missing streamer info.** ROOT reads a class it has compiled in without one,
   and the warning about it fires only when the file's `fVersion` differs from the
   running ROOT's, so a writer stamping the current release suppresses it. Every
...
A writer may choose the following freely. ROOT's own choices are given in the
writing documents only so that a byte comparison against a ROOT-written file stays
...
**An update adds nothing to this page.** A file that was reopened eleven times
satisfies the invariants above and no others, because nothing in the result
records that it was reopened.
[Writing a file §13](../06-writing/WritingFiles.md#13-updating-an-existing-file)
is a procedure, not a new set of rules. The one invariant an update should check
and a create need not is `fBEGIN`, above: a writer that creates files satisfies
it by construction, while one that reopens them takes the number from the file.

`spec/06-writing/` marks each field **fixed**, **derived** or **free** for this
reason.
