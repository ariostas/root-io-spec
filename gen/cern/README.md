...
| `root/roottest/root/io/evolution/issue-8083/stringarray.old.root` | 6.25/01 | `fStreamerType` 320 (`kSTL + kOffsetL`) on a `std::string` array whose element stores 500 ([TBranchElement](../../spec/04-ttree/TBranchElement.md)) | 0 failures, 11 of 11 branch-baskets. Until 2026-09-23 `ReadingEntries 8.5` failed on `obj.fOutputNames[4]`: ROOT writes the array under one byte count and `rootfile.py` read one string ([Collections §11.1](../../spec/02-serialization/Collections.md)) |
| `root/roottest/root/tree/selector/Event1.root` | 3.03/09 | A member-wise `TClonesArray` whose counter `fNsp` is itself a column, one count per track ([Collections §4.1](../../spec/02-serialization/Collections.md)) | 0 failures, 25 of 27. Until 2026-09-24 `ReadingEntries 8.5`: the counter was never found |
| `root/roottest/root/tree/addresses/data/S_1_104_qgsjet_100_1.KGrec.root` | 5.10/00 | Empty entries of split collection members written as 0 bytes, before 5.32/00; a `vector` of an enum with `fCtype` 0 ([ReadingEntries](../../spec/04-ttree/ReadingEntries.md), [Collections](../../spec/02-serialization/Collections.md)) | 0 failures, 356 of 356. Until 2026-09-24, 54 `ReadingEntries 8.5` |
| `root/roottest/root/tree/addresses/memleak.root` | 5.17/06 | A split `kStreamLoop` branch whose counter is a sibling, with `fBranchCount` unset before 5.27/06 ([ReadingEntries §4](../../spec/04-ttree/ReadingEntries.md)) | 0 failures, 24 of 26 |
| `root/roottest/root/io/vararyobj/varyingArray_51508.root` | 5.15/08 | `fType` 521 (`kStreamLoop + kOffsetL`), and a pre-5.16/00 loop of pointers holding bare objects ([ElementTypes](../../spec/02-serialization/ElementTypes.md)) | 0 failures, 7 of 7. Until 2026-09-24 two `ElementTypes 11.1` and two `ReadingEntries 8.5` |
| `root/roottest/root/meta/MakeProject/small_aod.pool.root` | 5.22/00 | ATLAS POOL file: empty split-collection entries written as 0 bytes ([ReadingEntries](../../spec/04-ttree/ReadingEntries.md)) | 0 failures, 4339 of 4339. Until 2026-09-24, 350 `ReadingEntries 8.5` |
| `root/roottest/root/meta/MakeProject/CMSSW_3_1_0_pre11-RelValZTT-default-copy.root` | 5.22/00 | Element type names without default template arguments, and a multimap stored as `fSTLtype` 4 before 5.24/00 ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md), [Collections](../../spec/02-serialization/Collections.md)) | 0 failures, 1298 of 1298. Until 2026-09-24 eight `StreamerDriven 10.5` and one `Collections 14.10` |
| `root/roottest/root/meta/evolution/checksum_v5.root` | 5.34/18 | Element type names spelled with ROOT typedefs (`UserTmplt<Int_t>`), and a class-scoped typedef nothing in the file resolves ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md), [Collections §9](../../spec/02-serialization/Collections.md)) | 1 failure, `Collections 14.6`: the file's fault, and ROOT cannot read it without a dictionary either |
| `root/roottest/root/meta/evolution/checksum_v53418.root` | 5.34/18 | As `checksum_v5.root` | 1 failure, `Collections 14.6`, as `checksum_v5.root` |
| `root/roottest/root/meta/evolution/checksum_v6.root` | 5.99/06 | Typedef spellings in element type names ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md)) | 0 failures |
| `root/roottest/root/meta/tclass/pairs_v5.root` | 5.34/19 | Typedef spellings in a `pair`'s element type names ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md)) | 0 failures |
| `root/roottest/root/meta/evolution/version5/nestedColl.root` | 5.34/33 | A collection's value type recorded without its namespace (`vector<OtherInner>`), resolved by checksum ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md)) | 0 failures |
| `root/roottest/root/io/emulated/lariat-si.root` | 6.11/01 | `unsigned long long` in an element type name where the info says `ULong64_t` ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md)) | 0 failures |
| `root/roottest/root/meta/evolution/foreignVec.root` | 5.34/18 | A header with the large-file flag and `fUnits` 4 in a 7 KB file of narrow keys ([FileHeader](../../spec/01-container/FileHeader.md)) | 0 failures. Until 2026-09-24 `FileHeader 10.9` and `10.10` |
| `root/roottest/root/tree/cloning/files/output_Coulomb_LER_study_10.root` | 6.07/01 | Fast-merged split parents ([TBranch §7](../../spec/04-ttree/TBranch.md)); a `TTreeIndex` copied untrimmed ([Auxiliary](../../spec/04-ttree/Auxiliary.md)); an element type name no class has, `CalcMeanCov<2,value_type>` ([StreamerInfo](../../spec/02-serialization/StreamerInfo.md)) | 2 failures, `StreamerDriven 10.5`: the unresolvable name, the file's fault, and a `TGlobal` info whose base `TDictionary` has none, not yet explained |
| `root/roottest/root/tree/cloning/files/output_Coulomb_LER_study_small.root` | 6.13/01 | The output of `exectrim.C`: an untrimmed `TTreeIndex` of `fN` 320 on a 10-entry tree ([Auxiliary](../../spec/04-ttree/Auxiliary.md)) | 0 failures. Until 2026-09-24 `Auxiliary 8.3` |
| `root/roottest/root/dataframe/bigFile.root` | 6.17/01 | Split parents with `fBasketEntry[0]` 20 from three fast-merged inputs, before 6.22/08 ([TBranch §7](../../spec/04-ttree/TBranch.md)); 24 KB, not a large file | 0 failures, 84 of 84 |
| `root/roottest/root/dataframe/ship_ROOT_9674.root` | 6.17/01 | Folder branches (`kBranchFolder`) renamed after construction: `MCTrack` titled `cbmroot.Stack.MCTrack_` ([Splitting §3.3](../../spec/04-ttree/Splitting.md)) | 0 failures, 190 of 190. Until 2026-09-24 ten `Splitting 8.3` |
| `root/roottest/root/io/evolution/versions/lhcb/lhcb.root` | 5.17/07 | A split node of a class whose info lists no element, with 0-byte entries ([Splitting](../../spec/04-ttree/Splitting.md)); fast-cloned split parents ([TBranch §7](../../spec/04-ttree/TBranch.md)) | 0 failures, 11 of 11 |
| `root/roottest/root/io/clones/v5formula_clones.root` | 5.34/30 | A tree written with no file behind it: every basket embedded, in slots 0 to `fWriteBasket` ([TBranch §5](../../spec/04-ttree/TBranch.md)) | 0 failures, 5 of 13; the 8 `TBranchObject` baskets are skipped, a reader gap |
| `root/roottest/root/io/TFile/geodemo.root` | 5.34/35 | 73 `TStreamerBase` elements, every one with its checksum in `fMaxIndex[1]`: the checksum was backported to 5.34/19 ([StreamerInfo §9.1](../../spec/02-serialization/StreamerInfo.md)) | 0 failures |
| `root/roottest/root/tree/friend/dat_001.root` | 4.04/02 | A basket read back from its record and streamed again in slot 11 ([TBranch §5.1](../../spec/04-ttree/TBranch.md)); `dat_002.root` and `dat_003.root` are the same shape | 0 failures, 130 of 130. Until 2026-09-24, 10 `TBranch 11.9` in each |
| `root/roottest/root/tree/friend/dat_002.root` | 4.04/02 | As `dat_001.root` | 0 failures, 130 of 130 |
| `root/roottest/root/tree/friend/dat_003.root` | 4.04/02 | As `dat_001.root` | 0 failures, 130 of 130 |
| `root/roottest/root/treeproxy/stlvec/tlorentzvec.root` | 5.27/01 | Seven collection count branches titled `_` whose members say `[muon4mom_]`, which no ROOT constructor produces ([Splitting §3.3](../../spec/04-ttree/Splitting.md)) | 28 failures, `Splitting 8.4`, kept: the writer is unknown (`PLAN.md` §8.16) |
