...
**five** baskets, and two closed cluster ranges -- and it reproduces it
*exactly*: all five basket records and the `TTree` record are byte-identical to
...
policy values are inputs here for the same reason, and both are why the records
...
The file's name and title are chosen so the comparison is possible at all:
...
def build() -> bytes:
    ...
    for i, (upto, size) in enumerate(FLUSHES):
        ...
        # changing it is what closes the previous range (WritingTrees.md 7.4).
