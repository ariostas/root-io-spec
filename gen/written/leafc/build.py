...
branch, three entries, one basket each. Both basket records and the `TTree`
record are byte-identical to ROOT's, keys included, once the fixed timestamp is
masked.

Both paths are 23 characters so that this is possible: a branch stores its
baskets' offsets, so a one-byte shift anywhere before them would change the
`TTree` record.

The three strings are the three cases: the short counted form, an empty one that
