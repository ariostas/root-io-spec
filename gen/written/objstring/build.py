...
`StreamerInfo` record, which is legal (`FileHeader.md` invariant 8) and which
`spec/06-writing/WritingFiles.md` 8 measures the consequences of.

The name is passed as the repo-relative path for the same reason
`tools/generate.py` does it: a key carries the file's name, so an absolute path
