...
compared byte for byte with the one ROOT wrote** -- the subdirectory records
included, which is the only way to check `fNbytesName`, `fSeekParent` and the
in-place `fSeekKeys` against anything but this project's own reading of the
source.

Two choices exist purely to make that comparison possible, and neither is a
format requirement:

* the file's name is 31 characters, exactly as long as
...
What is left over after that is the three UUIDs, which are free
(`spec/01-container/Directory.md` 4.5), and nothing else: `tools/test_write.py`
asserts the rest of the file is identical.
"""
...
#: title -- so a key ROOT writes for one carries this, not the string.
