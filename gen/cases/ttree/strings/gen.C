...
/// bytes and its basket must carry an entry-offset array -- a TLeafC forces
...
///
/// Three entries and no compression, so the whole of both baskets can be
/// asserted byte for byte, and so this file can be compared with the one
