...
///
/// What the bytes show is two frames in a row before the content's own members:
/// the object slot's byte count and class record, then the content class's own
/// byte count and version word. The second is the class frame every object
/// carries, not a second collection frame -- which is what an outside review of
/// this specification read it as (issue #1 item 10, seen on
/// RooVectorDataStore::RealVector, whose class version is also 1).
///
