/// The other half of `classes/tarray`: a file that does have a TStreamerInfo
/// for a concrete TArray, so that TArray.md 2's "wrong by one byte" claim is
/// checked against real bytes rather than asserted.
///
...
/// only there to hold the branch; TBasket and TBranch have cases of their own.
