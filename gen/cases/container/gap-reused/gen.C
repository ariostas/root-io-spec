...
/// used again, which is the other half of the allocator and the half a writer
/// has to implement. Three placements happen here, and all three are visible in
/// the finished file:
///
...
///     behind. `exact` ends up at the very offset its first version had;
...
///
/// The "overwrite" option is what makes this controllable: it frees the old key
/// *before* allocating the new one (TDirectoryFile.cxx:1977-1985), so the space
...
/// that the restored record is exactly the size of the one it replaces.
void gen(const char *out)
{
   ...
   // them; the restore frees those 125 back, which coalesces into the original
   // 248 -- and 248 is exactly what it needs, so the entry is removed.
   ...
   // And that is a record of its own: `lodger` is small enough to fit what snug
   // released, so it is placed inside a dead record's span rather than at the
   // end of the file. Nothing connects it to snug -- the allocator matches on
   // size and address, never on identity.
