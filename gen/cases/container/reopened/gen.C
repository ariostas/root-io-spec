/// A file **reopened** and added to, which is the other way a ROOT file comes
/// to exist.
///
/// Everything before `Close()` on the first file is a create; everything after
/// `TFile::Open(out, "UPDATE")` is the procedure this case exists for, and the
/// three writes that follow are the three options `TObject::Write` offers, each
/// of which resolves differently:
///
...
///     (TDirectoryFile.cxx:1986, :2004-2007) -- so the new record cannot land on
///     the old one's bytes, and its key is appended while the old key is still
///     in the list, which is what makes the cycle advance. `overwrite` is the
///     same two operations in the other order and is what `container/gap-reused`
///     shows.
///
/// What the close then does is the point of the case: the key list and the
/// free-segment record are both **reallocated**, each freeing its old span
