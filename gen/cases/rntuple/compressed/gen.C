...
///
/// Every other RNTuple case here calls `opts.SetCompression(0)`, for a good
/// reason: it keeps the envelopes readable in place so the assertions are about
/// the format rather than about a codec. The cost of that choice was invisible
/// until 2026-09-22 -- with compression off, an `RBlob` payload is raw and the
/// container's compression invariants never run on one. With compression on they
/// do, and they fail, because a sealed page is `blocks || 8-byte XXH3 checksum`
/// and the checksum lies outside `fObjLen` and past the end of the block chain
...
/// to find; a page of distinct values would be stored raw and prove nothing.
