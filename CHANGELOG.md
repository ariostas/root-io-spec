...
- **Releases are dated now, not numbered.** CalVer, `YYYY.MM.DD`, cut at a
  ...
  state a version of their own -- [spec/index.md](spec/index.md) and the site
  config carry the **ROOT** release instead, which is the number `check_pin.py`
  and `check_citations.py` can hold in place. `v0.1.0` is the last release under
  the old scheme and stays where it is.
